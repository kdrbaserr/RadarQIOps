...
class PipelineTests(unittest.TestCase):
    def test_train_and_evaluate_round_trip(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            features = np.array([[0, 0], [0, 1], [10, 10], [10, 11], [1, 0], [11, 10]], dtype=np.float32)
            ...
            (root / "model.yaml").write_text(json.dumps({"artifact_path": str(root / "model.npz")}), encoding="utf-8")
            (root / "train.yaml").write_text(json.dumps({
                "seed": 4,
            ...
            (root / "evaluate.yaml").write_text(json.dumps({
